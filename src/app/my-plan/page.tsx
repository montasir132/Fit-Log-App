...
const MyPlanPage = () => {
  ...
  } = useContext(ExerciseContext) as IExerciseContextType;
  ...
  const activeExercises =
    activeTab === "today" ? sortedAddTodayPlan : sortedSaveForLater;
        ...
        <div className="mt-6 flex flex-col gap-4 sm:mt-8 md:flex-row md:items-center md:justify-between">
          ...
          <div className="w-full rounded-2xl border border-[#24282e] bg-[#171a20] p-1 sm:w-fit">
            <div className="grid grid-cols-2 sm:flex">
              ...
              >
                {`Today's Plan`}
                <span className="ml-2 text-xs">({addTodayPlan.length})</span>
              </button>
              ...
              >
                Saved
                <span className="ml-2 text-xs">({saveForLater.length})</span>
              </button>
            </div>
          </div>
          {/* Sort */}
                ...
                <div className="space-y-4">
                  {sortedAddTodayPlan.map((exercise: IExercise) => (
                    <MyPlanCard
                      key={exercise.id}
                      exercise={exercise}
                      onMarkDone={() => markAsDoneFromTodayPlan(exercise.id)}
                      onRemove={() => removeFromTodayPlan(exercise.id)}
                    />
                  ))}
                </div>
              ) : (
                ...
                <div className="space-y-4">
                  {sortedSaveForLater.map((exercise: IExercise) => (
                    <MyPlanCard
                      key={exercise.id}
                      exercise={exercise}
                      onMarkDone={() => markAsDoneFromSaveForLater(exercise.id)}
                      onRemove={() => removeFromSaveForLater(exercise.id)}
                    />
                  ))}
                </div>
              ) : (
